        ...
        <article class="p-6 mb-1 text-base bg-white rounded-lg ">
            <footer class="flex justify-between items-center mb-1">
                <div class="flex items-center">
                    <p class="inline-flex items-center mr-3 text-sm text-gray-900">
                        @if ($comment->user->avatar)
                            <img class="m-2 w-10 h-10 rounded-full" src="{{ asset('storage/' . $comment->user->avatar) }}"
                                alt="{{ $comment->user->name }}">
                        @else
                            <img class="m-2 w-10 h-10 rounded-full" src = 
                            "{{$comment->user->avatar()}}"
                                alt="{{ $comment->user->name }}">
                        @endif
                ...
                <div class="relative">
                    ...
                    @if ($showOptions)
                        <div
                            class="absolute z-10 top-full right-0 mt-1 w-36 bg-white rounded divide-y divide-gray-100 shadow dark:divide-gray-600">
                            <ul class="py-1 text-sm">
                                ...
                                @can('destroy', $comment)
                                    <li>
                                        <button x-on:click="confirmCommentDeletion" x-data="{ confirmCommentDeletion() { if (window.confirm('{{ __('commentify::commentify.comments.delete_confirm') }}')) { @this.call('deleteComment') } } }"
                                            class="block w-full text-left py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
